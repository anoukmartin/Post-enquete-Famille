...
editor: visual
params:
  id_individu: "DOMINIQUE 1979-03-14"
...
#Recharger liens 
liens <- readRDS(here("1_data", "processed", "liens.RDS")) %>%
    filter(identifiant == id_individu) 
...
knitr::kable(
  liens %>%
    select(prenom, age, sexe, libelle_relation) %>%
    bind_rows(data.frame(prenom = BIEF$prenom, 
                             age = as.double(BIEF$AGE), 
                             sexe = str_sub(BIEF$sexe, 1, 1),  
                             libelle_relation = "EGO"),
              .),
