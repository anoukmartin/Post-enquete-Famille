...
url <- "https://opendata.iledefrance.fr/api/explore/v2.1/catalog/datasets/communes-et-arrondissements-municipaux-annee-en-cours/exports/shp?lang=fr&timezone=Europe%2FBerlin&use_labels=true"
...
idf_sf$codecommune <- idf_sf$Code_commun.1 

coordonnees$Code_commune[!coordonnees$Code_commune %in% idf_sf$codecommune]
...
ggplot() +
  ...
  geom_text_repel(
    data = centres,
    aes(x = X, y = Y, label = paste0(str_to_title(Commune__Ar), " [", nb_individus, "]")),
    size = 2.5,
    max.overlaps = Inf,
...
plot_categorique(coordonnees, "iln", titre = "Lieu de naissance")

```
## Acendance migratoire
